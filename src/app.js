  ...
  return new AirDatepicker("#datepicker", {
    ...
    onRenderCell: ({ date, cellType }) => {
      if (cellType === 'month') {
        ...
        return {
          disabled: month < 6 || month > 9,
          classes: month < 6 || month > 9 ? 'hidden' : ''
        };
      }
    },
    locale: {
...
function updateTicketSummary() {
  ...
  if (dateInput) {
    ...
    if (!isNaN(selectedDate)) {
      fecha = selectedDate.toLocaleDateString('es-MX', { month: 'long', day: 'numeric' });
    }
  }
  ...
  if (tickets && fecha && time) {
    summary.innerHTML = `<span>${fecha}, ${time} hrs.</span><span>${tickets}</span>`;
  } else if (tickets && fecha) {
    label.innerText = "Selecciona un horario";
    summary.innerHTML = `<span>${fecha}</span><span>${tickets}</span>`;
  } else if (tickets) {
    label.innerText = "Selecciona un fecha y horario";
    summary.innerHTML = `<span>—</span><span>${tickets}</span>`;
  }

  // Update confirmation section if exists
  if (document.getElementById("confirmationDate")) {
    document.getElementById("confirmationDate").textContent = fecha ?? "";
